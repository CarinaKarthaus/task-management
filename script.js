...
function loadTasksMatrix() {
    loadAllTasks();
    for (i = 0; i < allTasks.length; i++) {
        compileTaskMatrixHTML("do-blue-box",i+1);
        assignCategory(allTasks[i].category, i+1);
        console.log(`task${i}`);
    }
}
//HTML code for each task to add to the matrix
function compileTaskMatrixHTML(id, taskId) {
...
function assignCategory(category, id) {
